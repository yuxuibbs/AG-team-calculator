...
HIGHEST_STATES_PREZ_ELEMENTARY = 0
HIGHEST_STATES_PREZ_MIDDLE = 0
HIGHEST_STATES_PROP_ELEMENTARY = 0
HIGHEST_STATES_PROP_MIDDLE = 0
NUM_MIDDLE = 0
NUM_ELEMENTARY = 0
...
################################################################################
# States scores
def get_states_info(grade, playerName, dictionary_name, label, data):
    qualified_players[grade][playerName][dictionary_name][label].append(int(data))

def insert_states_data(row, grade):
    playerName = row['FirstName'] + " " + row['LastName']
    if playerName in qualified_players[grade]:
...
def insert_rank_data(row, grade):
    ...
    if playerName in qualified_players[grade]:
        ...
        get_rank_data(grade, playerName, 'states_rank', float(row['States']))
...
################################################################################
# Saving stuff
def keep_states_participants():
    to_delete = []
    for grade in qualified_players:
        for person in qualified_players[grade]:
            if 'States' not in qualified_players[grade][person]:
                to_delete.append(person)
    for grade in qualified_players:
        for person in to_delete:
            if person in qualified_players[grade]:
                del qualified_players[grade][person]

def clean_states_data(person_data, grade):
    global HIGHEST_STATES_PREZ_ELEMENTARY
    global HIGHEST_STATES_PREZ_MIDDLE
    global HIGHEST_STATES_PROP_ELEMENTARY
    global HIGHEST_STATES_PROP_MIDDLE

    person_data['States']['states_prez_total'] = int(np.sum(person_data['States']['Prez']))
    person_data['States']['states_prop_total'] = int(np.sum(person_data['States']['Prop']))

    if grade == 6:
        if person_data['States']['states_prez_total'] > HIGHEST_STATES_PREZ_ELEMENTARY:
            HIGHEST_STATES_PREZ_ELEMENTARY = person_data['States']['states_prez_total']
        if person_data['States']['states_prop_total'] > HIGHEST_STATES_PROP_ELEMENTARY:
            HIGHEST_STATES_PROP_ELEMENTARY = person_data['States']['states_prop_total']
    else:
        if person_data['States']['states_prez_total'] > HIGHEST_STATES_PREZ_MIDDLE:
            HIGHEST_STATES_PREZ_MIDDLE = person_data['States']['states_prez_total']
        if person_data['States']['states_prop_total'] > HIGHEST_STATES_PROP_MIDDLE:
            HIGHEST_STATES_PROP_MIDDLE = person_data['States']['states_prop_total']
...
def calculate_sweeps(person_data, grade):
    # EQ + OS + Ling + Prez
    person_data['Friday_sweeps'] = 0
    person_data['Friday_sweeps_calculation'] = {}
    # Friday
    for game in CUBE_GAMES:
        # scale to 24
        if game in person_data['Friday_Tournaments']:
            ...
            person_data['Friday_sweeps_calculation'][game] = scaled_score
    person_data['Friday_sweeps_no_prez'] = person_data['Friday_sweeps']
    # Prez
    ...
    for score in range(3):
        person_data['States_sweeps'] += person_data['States_sweeps_scores'][score][0]
    # adjusted states sweeps
    if grade == 6:
        person_data['adjusted_States_sweeps'] = \
            person_data['States_sweeps'] + \
            (((NUM_ELEMENTARY * 2) - person_data['Rankings']['total_rank']) * 2) + \
            (HIGHEST_MEAN_TABLE_ELEMENTARY - person_data['Saturday_Tournaments']['saturday_mean_table']) + \
            (person_data['Saturday_Tournaments']['saturday_top_3_mean']) + \
            person_data['States']['TableAdjustment'] * 2
    else:
        person_data['adjusted_States_sweeps'] = \
            person_data['States_sweeps'] + \
            (((NUM_MIDDLE * 2) - person_data['Rankings']['total_rank']) * 2) + \
...
def clean_data():
    for grade, data in qualified_players.items():
        for person, person_data in data.items():
            ...
            clean_states_data(person_data, grade)
    for grade, data in qualified_players.items():
        for person, person_data in data.items():
...
get_states_data('2018-2019/states.csv')
